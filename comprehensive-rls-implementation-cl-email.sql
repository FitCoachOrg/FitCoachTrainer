-- Comprehensive RLS Implementation for FitCoachTrainer (cl_email version)
-- This script implements relationship-based access control using cl_email schema
...
-- Phase 2: Create comprehensive RLS policies (cl_email version)
...
-- 3. CLIENT TABLE POLICIES (using cl_email)
...
CREATE POLICY "Trainers can view their clients" ON client
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE tcw.cl_email = client.cl_email  -- Using cl_email instead of client_id
            AND t.trainer_email = auth.uid()::text
        )
    );
...
-- 4. ACTIVITY_INFO TABLE POLICIES (using cl_email)
...
CREATE POLICY "Trainers can view their clients activity" ON activity_info
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM trainer_client_web tcw
            JOIN trainer t ON tcw.trainer_id = t.id
            JOIN client c ON c.client_id = activity_info.client_id
            WHERE tcw.cl_email = c.cl_email  -- Using cl_email through client table
            AND t.trainer_email = auth.uid()::text
        )
    );
...
-- 5. MEAL_INFO TABLE POLICIES (using cl_email)
...
CREATE POLICY "Trainers can view their clients meals" ON meal_info
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM trainer_client_web tcw
            JOIN trainer t ON tcw.trainer_id = t.id
            JOIN client c ON c.client_id = meal_info.client_id
            WHERE tcw.cl_email = c.cl_email  -- Using cl_email through client table
            AND t.trainer_email = auth.uid()::text
        )
    );
...
-- 6. CLIENT_ENGAGEMENT_SCORE TABLE POLICIES (using cl_email)
...
CREATE POLICY "Trainers can view their clients engagement" ON client_engagement_score
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM trainer_client_web tcw
            JOIN trainer t ON tcw.trainer_id = t.id
            JOIN client c ON c.client_id = client_engagement_score.client_id
            WHERE tcw.cl_email = c.cl_email  -- Using cl_email through client table
            AND t.trainer_email = auth.uid()::text
        )
    );
...
CREATE INDEX IF NOT EXISTS idx_trainer_client_web_cl_email ON trainer_client_web(cl_email);
