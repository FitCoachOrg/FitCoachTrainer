...
// API key management - Force using Key 2
const API_KEYS = [
    process.env.VITE_CEREBRAS_API_KEY,
    process.env.CEREBRAS_API_KEY2
].filter(key => key && key !== 'your-cerebras-api-key-here');

// Start with API Key 2 (index 1)
let currentApiKeyIndex = 1; // Force using the second API key
...
// Function to switch to next API key (fallback to key 1 if needed)
...
function createBatches(exercises, batchSize = 30) { // Reduced batch size for better reliability
    const batches = [];
        ...
        try {
            ...
            const response = await fetch('https://api.cerebras.ai/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    temperature: 0.1,
                    max_tokens: 3000 // Reduced for smaller batches
                })
            });
...
// Main function to continue processing with Key 2
async function main() {
    ...
    console.log(`Starting with API Key ${currentApiKeyIndex + 1} (${currentApiKeyIndex === 0 ? 'Primary' : 'Secondary'})`);
    ...
    const outputFile = 'attached_assets/exercises_standardized_cerebras_complete_key2.csv';
    ...
    console.log('Continuing exercise standardization process with API Key 2...\n');
    ...
    try {
        ...
        // Process remaining exercises with smaller batches
        const batches = createBatches(remainingExercises, 30); // Smaller batches for better reliability
        const allProcessedExercises = [...processedExercises]; // Start with already processed
        
        console.log(`Processing ${remainingExercises.length} exercises in ${batches.length} batches of 30...`);
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            while (retryCount < maxRetries && !standardizedBatch) {
                ...
                if (!standardizedBatch) {
                    ...
                    if (retryCount < maxRetries) {
                        const delay = 15000; // Increased delay between retries
            ...
            if (standardizedBatch) {
                ...
                if (allProcessedExercises.length > 0) {
                    const emergencyFile = `attached_assets/exercises_standardized_cerebras_emergency_key2_${Date.now()}.csv`;
            ...
            if (i < batches.length - 1) {
                const delay = 5000; // Increased delay between batches
                console.log(`Waiting ${delay / 1000} seconds before next batch...`);
                await new Promise(resolve => setTimeout(resolve, delay));
            }
            
            if ((i + 1) % 5 === 0) { // Save progress more frequently
                const progressFile = `attached_assets/exercises_standardized_cerebras_progress_key2_${allProcessedExercises.length}.csv`;
