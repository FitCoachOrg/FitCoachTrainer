-- Todo List Table Schema for FitCoachTrainer
...
CREATE TABLE IF NOT EXISTS todos (
    ...
    CONSTRAINT todos_due_date_future CHECK (due_date IS NULL OR due_date >= created_at)
);
...
/*
INSERT INTO todos (trainer_id, title, client_id, priority, due_date, category) VALUES
    (auth.uid(), 'Review client progress reports', NULL, 'high', NOW() + INTERVAL '1 day', 'work'),
    (auth.uid(), 'Update training programs', NULL, 'medium', NOW(), 'work'),
    (auth.uid(), 'Schedule team meeting', NULL, 'low', NOW() + INTERVAL '7 days', 'meetings'),
    (auth.uid(), 'Order new equipment', NULL, 'medium', NOW() + INTERVAL '3 days', 'equipment');
*/
