...
function createBatches(exercises, batchSize = 100) {
    const batches = [];
...
// Function to process exercises in batches
async function processExercisesInBatches(exercises, batchSize = 100) {
    const batches = createBatches(exercises, batchSize);
    const allProcessedExercises = [];
    
    console.log(`Processing ${exercises.length} exercises in ${batches.length} batches...`);
    
    for (let i = 0; i < batches.length; i++) {
        console.log(`Processing batch ${i + 1}/${batches.length} (${batches[i].length} exercises)...`);
        
        const standardizedBatch = await standardizeExercisesWithLLM(batches[i]);
        ...
        if (standardizedBatch) {
            ...
            console.log(`✗ Batch ${i + 1} failed, using original data`);
            const fallbackBatch = mergeExerciseData(batches[i], []);
            allProcessedExercises.push(...fallbackBatch);
        }
...
async function main() {
    ...
    const outputFile = 'standardized_exercises.csv';
    ...
    console.log('Starting exercise standardization process...\n');
    ...
    try {
        ...
        const processedExercises = await processExercisesInBatches(exercises, 100);
