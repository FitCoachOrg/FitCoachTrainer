-- Fix trainer_exercises Table Issues (Safe Version)
-- Based on the error: null value in column "trainer_id" violates not-null constraint
-- This version doesn't drop shared functions that other tables depend on
...
-- 3. Drop only trainer_exercises specific triggers (safe)
...
-- 4. Drop only trainer_exercises specific functions (safe)
...
-- 6. Drop all existing policies for trainer_exercises only
...
-- 14. Create a trainer_exercises specific updated_at trigger (if needed)
-- Only create this if the table doesn't already have one
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'trainer_exercises' 
        AND trigger_name = 'update_trainer_exercises_updated_at'
    ) THEN
        CREATE TRIGGER update_trainer_exercises_updated_at
            BEFORE UPDATE ON trainer_exercises
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
        
        RAISE NOTICE 'Created updated_at trigger for trainer_exercises';
    ELSE
        RAISE NOTICE 'updated_at trigger already exists for trainer_exercises';
    END IF;
END $$;
