-- Trainer Schedule Management Schema (Final - Matches actual database structure)
-- This schema provides comprehensive schedule management for fitness trainers
-- Uses actual database tables: trainer (UUID), client (bigint), etc.
...
CREATE TABLE IF NOT EXISTS schedule_events (
    ...
    -- Trainer information (references actual trainer table with UUID)
    ...
    created_by UUID REFERENCES trainer(id),
    updated_by UUID REFERENCES trainer(id),
...
CREATE TABLE IF NOT EXISTS schedule_recurring_events (
    ...
    -- Trainer and client (references actual tables)
    trainer_id UUID NOT NULL REFERENCES trainer(id) ON DELETE CASCADE,
    trainer_email VARCHAR(255) NOT NULL,
    client_id BIGINT REFERENCES client(client_id) ON DELETE SET NULL,
    client_name VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS schedule_event_notes (
    ...
    -- Author
    created_by UUID NOT NULL REFERENCES trainer(id),
...
CREATE POLICY "Trainers can view their own events" ON schedule_events
    FOR SELECT USING (auth.uid()::text IN (
        SELECT trainer_email FROM trainer WHERE id = schedule_events.trainer_id
    ));
...
CREATE POLICY "Trainers can manage their recurring events" ON schedule_recurring_events
    FOR ALL USING (auth.uid()::text IN (
        SELECT trainer_email FROM trainer WHERE id = schedule_recurring_events.trainer_id
    ));
...
CREATE POLICY "Trainers can manage their event attendees" ON schedule_event_attendees
    FOR ALL USING (
        EXISTS (
            ...
            JOIN trainer t ON se.trainer_id = t.id
...
CREATE POLICY "Trainers can manage their event notes" ON schedule_event_notes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM schedule_events se
            JOIN trainer t ON se.trainer_id = t.id
            WHERE se.id = schedule_event_notes.event_id 
            AND t.trainer_email = auth.uid()::text
        )
    );
...
CREATE POLICY "Trainers can manage their availability" ON trainer_availability
    FOR ALL USING (auth.uid()::text IN (
        SELECT trainer_email FROM trainer WHERE id = trainer_availability.trainer_id
    ));

CREATE POLICY "Trainers can manage their time off" ON trainer_time_off
    FOR ALL USING (auth.uid()::text IN (
        SELECT trainer_email FROM trainer WHERE id = trainer_time_off.trainer_id
    ));
...
BEGIN
    ...
    IF TG_OP = 'INSERT' AND NEW.is_recurring = true THEN
        ...
        end_date := COALESCE(NEW.recurring_end_date, current_event_date + INTERVAL '1 year');
        
        -- Create events based on recurrence pattern
        WHILE current_event_date <= end_date AND occurrence_count < COALESCE(NEW.max_occurrences, 1000) LOOP
            event_start := current_event_date + (NEW.start_time::time);
            event_end := current_event_date + (NEW.end_time::time);
            ...
            CASE NEW.recurring_pattern
                WHEN 'daily' THEN
                    current_event_date := current_event_date + INTERVAL '1 day';
                WHEN 'weekly' THEN
                    current_event_date := current_event_date + INTERVAL '1 week';
                WHEN 'monthly' THEN
                    current_event_date := current_event_date + INTERVAL '1 month';
                WHEN 'yearly' THEN
                    current_event_date := current_event_date + INTERVAL '1 year';
                ELSE
                    current_event_date := current_event_date + INTERVAL '1 day';
            END CASE;
