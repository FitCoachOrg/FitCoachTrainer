...
async function main() {
    ...
    const outputFile = 'attached_assets/test_standardized_cerebras.csv';
    ...
    console.log('Starting test exercise standardization with Cerebras API...\n');
    ...
    try {
        ...
        // Process exercises with Cerebras API
        const standardizedExercises = await standardizeExercisesWithCerebras(testExercises);
        ...
        if (standardizedExercises) {
            ...
            console.log('\nIf the test looks good, you can run the full processing with:');
            console.log('npm run cerebras-standardize');
        } else {
            console.log('Failed to standardize exercises - all API keys failed');
        }
